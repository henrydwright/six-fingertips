...
public class AgentService : IAgentService
{
    ...
    public async Task<AgentResponse> ProcessUserInputAsync(string userInput)
    {
        try
        {
            ...
            // Create agent
            PersistentAgent agent = await CreateAgent();
            ...
            _logger.LogInformation("Run {RunId} completed, getting messages from {ThreadId}", run.Value.Id, thread.Id);
            ...
            //  look into the JSON by hand. Call a method to go do all of that!
            List<ToolFunctionCall> toolCalls = await GetToolCalls(thread, run);

            // Get the first message (which should be the assistant's response)
            var firstMessage = messages.First();

            // Tidy up the agents by deleting this one
            _logger.LogInformation("Deleting agent {AgentId}", agent.Id);
            ...
            foreach (MessageContent content in contentItems)
            {
                ...
                else if (contentType == typeof(MessageImageFileContent) && content != null)
                {
                    ...
                    if (fileId == "")
                        continue;
                    string imageMarkdown = await GetImageContentsAsMarkdownString(fileId);
                    fullAgentResponse += "\r\n\r\n" + imageMarkdown + "\r\n\r\n";
                }
            }
            ...
            {
                return new AgentResponse("No response recieved", lifetimeUsage, toolCalls);
            }
            else
            {
                return new AgentResponse(fullAgentResponse, lifetimeUsage, toolCalls);
            }
    ...
    private async Task<string> GetImageContentsAsMarkdownString(string fileId)
    {
    ...
    private async Task<List<ToolFunctionCall>> GetToolCalls(PersistentAgentThread thread, Azure.Response<ThreadRun> run)
    {
        var toolFunctionCalls = new List<ToolFunctionCall>();
        ...
        foreach (RunStep runStep in runSteps.Reverse())
        {
            try
            {
                ...
                _logger.LogInformation("Getting run step details for {RunStepId}", runStep.Id);
                ...
                if (stepType == "tool_calls")
                {
                    ...
                    foreach (JsonElement toolCall in toolCalls)
                    {
                        ...
                        if (toolType == "openapi")
                        {
                            ...
                            toolFunctionCalls.Add(new ToolFunctionCall(name, arguments, output));
                        }
                        else if (toolType == "code_interpreter")
                        {
                            ...
                            toolFunctionCalls.Add(new ToolFunctionCall("code_interpreter", arguments, ""));
                        }
                        else
                        {
        ...
        return toolFunctionCalls;
    }

    private async Task<PersistentAgent> CreateAgent()
    {
        ...
        _logger.LogInformation("Created agent with ID {AgentId}", agent.Id);
        return agent;
    }
} 